...
$sql_etype= "SELECT Field_Name AS exam_name, Value AS etype FROM setting WHERE Class=52";
$query_etype=mysql_query($sql_etype,$conn);
$row_etype=mysql_fetch_assoc($query_etype);

$etype=$row_etype['etype'];
...
student_course.s8,student_course.s9,
	...
	WHERE student_course.etype ='$etype' AND  student_info.StudentID='$userid'
...
<?php do{  echo '<h3>'.$row_etype['exam_name'].'</h3>'; ?>

<table width="90%" border="2" cellpadding="5" class="table2" style="text-align:center">
  <tr>
    <th>AQ</th>
    <th>AK</th>
    <th>FQ</th>
  ...
  <tr>
    <?php do{ ?>
    <?php $gred_fail='<span style="color:red">';  $gred_ex='<span style="color:blue">';?>
    ...
    <td><?php if($row_at['gredasum']>0){ echo $row_at['gredasum'].'A'; }else{};?>
...
<p></p>
<?php }while($row_etype=mysql_fetch_assoc($query_etype)) ?>
</div>
