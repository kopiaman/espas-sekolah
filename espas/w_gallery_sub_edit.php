...
include("include/w_gallery_sub_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "w_gallery_sub_inline_edit.htm" : "w_gallery_sub_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["wig"] = urldecode(postvalue("editid1"));
$savedKeys["wig"] = urldecode(postvalue("editid1"));
...
//	processing sub_cat - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_sub_cat = $pageObject->getControl("sub_cat", $id);
		$control_sub_cat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sub_cat - end
	...
	//	do event
		...
		{		
			...
			//	processing sub_cat - begin
				...
				if($condition)
				{
					$control_sub_cat->afterSuccessfulSave();
				}
	//	processing sub_cat - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["wig"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_gallery_sub_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_gallery_sub_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("wig", $data)));
...
//	detail tables
	$showDetailKeys["w_pic"]["masterkey1"] = $data["wig"];		

	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["wig"]));


//	wig - 
	$value = $pageObject->showDBValue("wig", $data, $keylink);
	$showValues["wig"] = $value;
	$showFields[] = "wig";
		$showRawValues["wig"] = substr($data["wig"],0,100);

//	wcat - 
	$value = $pageObject->showDBValue("wcat", $data, $keylink);
	$showValues["wcat"] = $value;
	$showFields[] = "wcat";
		$showRawValues["wcat"] = substr($data["wcat"],0,100);

//	sub_cat - 
	$value = $pageObject->showDBValue("sub_cat", $data, $keylink);
	$showValues["sub_cat"] = $value;
	$showFields[] = "sub_cat";
		$showRawValues["sub_cat"] = substr($data["sub_cat"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "w_gallery_sub";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_gallery_sub_view.php?".$viewlink."'\"");
