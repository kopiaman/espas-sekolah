...
if($inlineadd == ADD_INLINE)
	$templatefile = "attend_creator_inline_add.htm";
else
	$templatefile = "attend_creator_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: attend_creator_add.php');
...
//	processing teach_day - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_teach_day = $pageObject->getControl("teach_day", $id);
		$control_teach_day->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing teach_day - end
...
//	processing teach_day - start
			...
			if($inlineAddOption)
			{
				$control_teach_day->afterSuccessfulSave();
			}
//	processing teach_day - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'attend_creator_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'attend_creator_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: attend_creator_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "attend_creator";
		$options['firstTime'] = 1;
