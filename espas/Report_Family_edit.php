...
include("include/Report_Family_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Family_inline_edit.htm" : "Report_Family_edit.htm";
...
//	processing Name1 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Name1 = $pageObject->getControl("Name1", $id);
		$control_Name1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Name1 - end
//	processing tel1 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_tel1 = $pageObject->getControl("tel1", $id);
		$control_tel1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tel1 - end
//	processing Name2 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Name2 = $pageObject->getControl("Name2", $id);
		$control_Name2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Name2 - end
//	processing tel2 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_tel2 = $pageObject->getControl("tel2", $id);
		$control_tel2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing Name1 - begin
				...
				if($condition)
				{
					$control_Name1->afterSuccessfulSave();
				}
	//	processing Name1 - end
			//	processing tel1 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_tel1->afterSuccessfulSave();
				}
	//	processing tel1 - end
			//	processing Name2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Name2->afterSuccessfulSave();
				}
	//	processing Name2 - end
			//	processing tel2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_tel2->afterSuccessfulSave();
				}
	//	processing tel2 - end
			//	processing darjah - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Family_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Family_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Name1"] = $evalues["Name1"];
	$data["tel1"] = $evalues["tel1"];
	$data["Name2"] = $evalues["Name2"];
	$data["tel2"] = $evalues["tel2"];
...
//	Name1 - 
	$value = $pageObject->showDBValue("Name1", $data, $keylink);
	$showValues["Name1"] = $value;
	$showFields[] = "Name1";
...
//	relation1 - 
	$value = $pageObject->showDBValue("relation1", $data, $keylink);
	$showValues["relation1"] = $value;
	$showFields[] = "relation1";
		$showRawValues["relation1"] = substr($data["relation1"],0,100);

//	Name2 - 
	$value = $pageObject->showDBValue("Name2", $data, $keylink);
	$showValues["Name2"] = $value;
	$showFields[] = "Name2";
		$showRawValues["Name2"] = substr($data["Name2"],0,100);

//	tel2 - 
	$value = $pageObject->showDBValue("tel2", $data, $keylink);
	$showValues["tel2"] = $value;
	$showFields[] = "tel2";
		$showRawValues["tel2"] = substr($data["tel2"],0,100);

//	relation2 - 
	$value = $pageObject->showDBValue("relation2", $data, $keylink);
	$showValues["relation2"] = $value;
	$showFields[] = "relation2";
		$showRawValues["relation2"] = substr($data["relation2"],0,100);

//	kelas - 
	$value = $pageObject->showDBValue("kelas", $data, $keylink);
	$showValues["kelas"] = $value;
	$showFields[] = "kelas";
		$showRawValues["kelas"] = substr($data["kelas"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Family";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Family_view.php?".$viewlink."'\"");
