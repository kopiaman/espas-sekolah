...
$layout->blocks["top"][] = "search";$page_layouts["student_upsra_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "student_upsra_search.htm";
...
$params['shortTableName'] = 'student_upsra';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("syear"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("syear")] = GetTableURL($pageObject->pSet->getLookupTable("syear"));
	
	$pageObject->fillFieldToolTips("syear");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("syear");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("syear_label","<label for=\"".GetInputElementId("syear", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("syear_label", true);
	
	$xt->assign("syear_fieldblock", true);
	$xt->assignbyref("syear_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("syear_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("syear_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_syear", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("syear");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"syear", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"syear", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
