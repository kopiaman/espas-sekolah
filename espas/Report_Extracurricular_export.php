...
include("include/Report_Extracurricular_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["Report_Extracurricular_export"] = $layout;
...
$xt->display("Report_Extracurricular_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Extracurricular.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Extracurricular.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Extracurricular.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Achievement"] = $pageObject->showDBValue("Achievement", $row);
			$values["Start_Date"] = $pageObject->showDBValue("Start_Date", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Extracurricular.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Achievement"] = $pageObject->getViewControl("Achievement")->showDBValue($row, "");
			$values["Start_Date"] = $pageObject->getViewControl("Start_Date")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Achievement"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Start_Date"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tarikh Mula").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Peringkat").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tarikh Akhir").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Tarikh Mula"."</td>";
		echo "<td>"."Peringkat"."</td>";
		echo "<td>"."Tarikh Akhir"."</td>";
		echo "<td>"."Status"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Achievement"]);
					else
						echo $values["Achievement"];
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Start_Date"]);
					else
