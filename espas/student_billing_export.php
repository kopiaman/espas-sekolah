...
$layout->blocks["top"][] = "export";$page_layouts["student_billing_export"] = $layout;
...
$xt->display("student_billing_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=student_billing.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_billing.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_billing.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["itemid"] = $pageObject->showDBValue("itemid", $row);
			...
			$values["balance"] = $pageObject->showDBValue("balance", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_billing.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["itemid"] = $pageObject->getViewControl("itemid")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["itemid"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Bil(RM)").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("No Bil").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Dibayar").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Status").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Baki").'</td>';	
	}
	else
	{
		echo "<td>"."Kelas"."</td>";
	...
	echo "</tr>";
		...
		$totalsFields[] = array('fName'=>"paid_amount", 'totalsType'=>'TOTAL', 'viewFormat'=>"Currency");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["paid_amount"] = $pageObject->getViewControl("paid_amount")->showDBValue($row, "");
					$values["sstatus"] = $pageObject->getViewControl("sstatus")->showDBValue($row, "");
					$values["balance"] = $pageObject->getViewControl("balance")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["itemid"]);
				else
					echo $values["itemid"];//echo htmlspecialchars($values["itemid"]); commented for bug #6823
	...
	echo "<td>";
						echo "Jumlah".": ";
	echo htmlspecialchars(GetTotals("paid_amount", $totals["paid_amount"]["value"], "TOTAL", $totals["paid_amount"]["numRows"], "Currency", PAGE_EXPORT));
	echo "</td>";
	echo "<td>";
		echo "</td>";
	echo "<td>";
