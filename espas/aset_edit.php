...
$templatefile = ($inlineedit == EDIT_INLINE) ? "aset_inline_edit.htm" : "aset_edit.htm";
...
//	processing cat - end
//	processing quantity - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_quantity = $pageObject->getControl("quantity", $id);
		$control_quantity->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing quantity - end
...
//	processing catatan - end
	...
	//	processing cat - end
			//	processing quantity - begin
				...
				if($condition)
				{
					$control_quantity->afterSuccessfulSave();
				}
	//	processing quantity - end
			//	processing sstatus - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: aset_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: aset_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["quantity"] = $evalues["quantity"];
...
//	quantity - 
	...
	$showFields[] = "quantity";
		$showRawValues["quantity"] = substr($data["quantity"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='aset_view.php?".$viewlink."'\"");
