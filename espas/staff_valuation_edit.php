...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_valuation_inline_edit.htm" : "staff_valuation_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["svid"] = urldecode(postvalue("editid1"));
$savedKeys["svid"] = urldecode(postvalue("editid1"));
...
//	processing rate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_rate = $pageObject->getControl("rate", $id);
		$control_rate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing rate - end
//	processing sdate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_sdate = $pageObject->getControl("sdate", $id);
		$control_sdate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sdate - end
//	processing remark - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_remark = $pageObject->getControl("remark", $id);
		$control_remark->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing rate - begin
				...
				if($condition)
				{
					$control_rate->afterSuccessfulSave();
				}
	//	processing rate - end
			//	processing sdate - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_sdate->afterSuccessfulSave();
				}
	//	processing sdate - end
			//	processing remark - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_remark->afterSuccessfulSave();
				}
	//	processing remark - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["svid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: staff_valuation_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_valuation_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["rate"] = $evalues["rate"];
	$data["sdate"] = $evalues["sdate"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("svid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["svid"]));


//	svid - 
	$value = $pageObject->showDBValue("svid", $data, $keylink);
	$showValues["svid"] = $value;
	$showFields[] = "svid";
		$showRawValues["svid"] = substr($data["svid"],0,100);
...
//	rate - 
	$value = $pageObject->showDBValue("rate", $data, $keylink);
	$showValues["rate"] = $value;
	$showFields[] = "rate";
		$showRawValues["rate"] = substr($data["rate"],0,100);

//	sdate - Short Date
	$value = $pageObject->showDBValue("sdate", $data, $keylink);
	$showValues["sdate"] = $value;
	$showFields[] = "sdate";
		$showRawValues["sdate"] = substr($data["sdate"],0,100);

//	remark - 
	$value = $pageObject->showDBValue("remark", $data, $keylink);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_valuation";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_valuation_view.php?".$viewlink."'\"");
