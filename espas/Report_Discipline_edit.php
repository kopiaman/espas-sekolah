...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Discipline_inline_edit.htm" : "Report_Discipline_edit.htm";
...
//	processing darjah - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_darjah = $pageObject->getControl("darjah", $id);
		$control_darjah->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing darjah - end
	...
	//	processing Comment - end
			//	processing darjah - begin
				...
				if($condition)
				{
					$control_darjah->afterSuccessfulSave();
				}
	//	processing darjah - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Discipline_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Discipline_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	darjah - 
	$value = $pageObject->showDBValue("darjah", $data, $keylink);
	$showValues["darjah"] = $value;
	$showFields[] = "darjah";
		$showRawValues["darjah"] = substr($data["darjah"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Discipline";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Discipline_view.php?".$viewlink."'\"");
