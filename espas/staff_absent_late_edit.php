...
include("include/staff_absent_late_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_absent_late_inline_edit.htm" : "staff_absent_late_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["abid"] = urldecode(postvalue("editid1"));
$savedKeys["abid"] = urldecode(postvalue("editid1"));
...
//	processing abdate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_abdate = $pageObject->getControl("abdate", $id);
		$control_abdate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing abdate - end
//	processing reason - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_reason = $pageObject->getControl("reason", $id);
		$control_reason->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing note - end
	...
	//	processing StaffID - end
			//	processing abdate - begin
				...
				if($condition)
				{
					$control_abdate->afterSuccessfulSave();
				}
	//	processing abdate - end
			//	processing reason - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_reason->afterSuccessfulSave();
				}
	//	processing reason - end
			//	processing note - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["abid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: staff_absent_late_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_absent_late_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["abdate"] = $evalues["abdate"];
	$data["reason"] = $evalues["reason"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("abid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["abid"]));


//	abid - 
	$value = $pageObject->showDBValue("abid", $data, $keylink);
	$showValues["abid"] = $value;
	$showFields[] = "abid";
		$showRawValues["abid"] = substr($data["abid"],0,100);
...
//	abdate - Short Date
	$value = $pageObject->showDBValue("abdate", $data, $keylink);
	$showValues["abdate"] = $value;
	$showFields[] = "abdate";
		$showRawValues["abdate"] = substr($data["abdate"],0,100);
...
//	reason - 
	$value = $pageObject->showDBValue("reason", $data, $keylink);
...
//	late - Checkbox
	$value = $pageObject->showDBValue("late", $data, $keylink);
	$showValues["late"] = $value;
	$showFields[] = "late";
		$showRawValues["late"] = substr($data["late"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_absent_late";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "staff_absent_late";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_absent_late_view.php?".$viewlink."'\"");
