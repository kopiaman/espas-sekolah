...
$layout->blocks["top"][] = "export";$page_layouts["user_export"] = $layout;
...
function ExportToXML($cipherer)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["role"] = $pageObject->showDBValue("role", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["role"] = $pageObject->getViewControl("role")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["role"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ic No").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Role").'</td>';	
	}
	else
	{
		echo "<td>"."Ic No"."</td>";
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["role"];//echo htmlspecialchars($values["role"]); commented for bug #6823
