...
$layout->blocks["top"][] = "export";$page_layouts["Report_Family_export"] = $layout;
...
$xt->display("Report_Family_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Family.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Family.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Family.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["kelas"] = $pageObject->showDBValue("kelas", $row);
			$values["sesi"] = $pageObject->showDBValue("sesi", $row);
			$values["Status"] = $pageObject->showDBValue("Status", $row);
			$values["Name1"] = $pageObject->showDBValue("Name1", $row);
			$values["tel1"] = $pageObject->showDBValue("tel1", $row);
			$values["Name2"] = $pageObject->showDBValue("Name2", $row);
			$values["tel2"] = $pageObject->showDBValue("tel2", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Family.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["kelas"] = $pageObject->getViewControl("kelas")->showDBValue($row, "");
			$values["sesi"] = $pageObject->getViewControl("sesi")->showDBValue($row, "");
			$values["Status"] = $pageObject->getViewControl("Status")->showDBValue($row, "");
			$values["Name1"] = $pageObject->getViewControl("Name1")->showDBValue($row, "");
			$values["tel1"] = $pageObject->getViewControl("tel1")->showDBValue($row, "");
			$values["Name2"] = $pageObject->getViewControl("Name2")->showDBValue($row, "");
			$values["tel2"] = $pageObject->getViewControl("tel2")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kelas"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Name1"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tel1"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Name2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tel2"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("No Tel Bapa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nama Ibu").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("No Tel Ibu").'</td>';	
	}
	else
	{
		echo "<td>"."Darjah"."</td>";
		...
		echo "<td>"."No Tel Bapa"."</td>";
		echo "<td>"."Nama Ibu"."</td>";
		echo "<td>"."No Tel Ibu"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["kelas"]);
				else
					echo $values["kelas"];//echo htmlspecialchars($values["kelas"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Name1"]);
					else
						echo $values["Name1"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tel1"]);
					else
						echo $values["tel1"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Name2"]);
					else
						echo $values["Name2"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tel2"]);
					else
						echo $values["tel2"];
			echo '</td>';
			echo "</tr>";
