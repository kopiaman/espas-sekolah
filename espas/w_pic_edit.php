...
$templatefile = ($inlineedit == EDIT_INLINE) ? "w_pic_inline_edit.htm" : "w_pic_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["picid"] = urldecode(postvalue("editid1"));
$savedKeys["picid"] = urldecode(postvalue("editid1"));
...
//	processing caption - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_caption = $pageObject->getControl("caption", $id);
		$control_caption->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing caption - end
	...
	//	processing files - end
			//	processing caption - begin
				...
				if($condition)
				{
					$control_caption->afterSuccessfulSave();
				}
	//	processing caption - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["picid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_pic_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_pic_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("picid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["picid"]));


//	picid - 
	$value = $pageObject->showDBValue("picid", $data, $keylink);
	$showValues["picid"] = $value;
	$showFields[] = "picid";
		$showRawValues["picid"] = substr($data["picid"],0,100);
...
//	caption - 
	$value = $pageObject->showDBValue("caption", $data, $keylink);
	$showValues["caption"] = $value;
	$showFields[] = "caption";
		$showRawValues["caption"] = substr($data["caption"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_pic_view.php?".$viewlink."'\"");
