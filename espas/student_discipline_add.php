...
add_nocache_headers();
include("include/student_discipline_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["student_discipline_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_discipline_inline_add.htm";
else
	$templatefile = "student_discipline_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_discipline_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing Activity - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Activity = $pageObject->getControl("Activity", $id);
		$control_Activity->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Activity - end
...
//	processing Date_Committed - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Date_Committed = $pageObject->getControl("Date_Committed", $id);
		$control_Date_Committed->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Date_Committed - end
//	processing Action_Taken - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Action_Taken = $pageObject->getControl("Action_Taken", $id);
		$control_Action_Taken->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Action_Taken - end
//	processing Comment - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Comment = $pageObject->getControl("Comment", $id);
		$control_Comment->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Comment - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_info")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["StudentID"]==""){
			$avalues["StudentID"] = prepare_for_db("StudentID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing Activity - start
			...
			if($inlineAddOption)
			{
				$control_Activity->afterSuccessfulSave();
			}
//	processing Activity - end
//	processing Level - start
			$inlineAddOption = true;
...
//	processing Date_Committed - start
			...
			if($inlineAddOption)
			{
				$control_Date_Committed->afterSuccessfulSave();
			}
//	processing Date_Committed - end
//	processing Action_Taken - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_Action_Taken->afterSuccessfulSave();
			}
//	processing Action_Taken - end
//	processing Comment - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_Comment->afterSuccessfulSave();
			}
//	processing Comment - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_discipline_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_discipline_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_discipline_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_info")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["StudentID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["Activity"]=@$avalues["Activity"];
	$defvalues["Level"]=@$avalues["Level"];
	$defvalues["Date_Committed"]=@$avalues["Date_Committed"];
	$defvalues["Action_Taken"]=@$avalues["Action_Taken"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Activity"))
		$xt->assign("Activity_fieldblock",true);
	else
		$xt->assign("Activity_tabfieldblock",true);
	$xt->assign("Activity_label",true);
	if(isEnableSection508())
		$xt->assign_section("Activity_label","<label for=\"".GetInputElementId("Activity", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Date_Committed"))
		$xt->assign("Date_Committed_fieldblock",true);
	else
		$xt->assign("Date_Committed_tabfieldblock",true);
	$xt->assign("Date_Committed_label",true);
	if(isEnableSection508())
		$xt->assign_section("Date_Committed_label","<label for=\"".GetInputElementId("Date_Committed", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Action_Taken"))
		$xt->assign("Action_Taken_fieldblock",true);
	else
		$xt->assign("Action_Taken_tabfieldblock",true);
	$xt->assign("Action_Taken_label",true);
	if(isEnableSection508())
		$xt->assign_section("Action_Taken_label","<label for=\"".GetInputElementId("Action_Taken", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Comment"))
		$xt->assign("Comment_fieldblock",true);
	else
		$xt->assign("Comment_tabfieldblock",true);
	$xt->assign("Comment_label",true);
	if(isEnableSection508())
		$xt->assign_section("Comment_label","<label for=\"".GetInputElementId("Comment", $id, PAGE_ADD)."\">","</label>");
...
//	Activity
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Activity", $data, $keylink);
		$showValues["Activity"] = $value;
		$showFields[] = "Activity";
	}	
//	Level
	$display = false;
...
//	Date_Committed
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Date_Committed", $data, $keylink);
		$showValues["Date_Committed"] = $value;
		$showFields[] = "Date_Committed";
	}	
//	Action_Taken
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Action_Taken", $data, $keylink);
		$showValues["Action_Taken"] = $value;
		$showFields[] = "Action_Taken";
	}	
//	Comment
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Comment", $data, $keylink);
		$showValues["Comment"] = $value;
		$showFields[] = "Comment";
	}	
		$showRawValues["ID"] = substr($data["ID"],0,100);
		$showRawValues["StudentID"] = substr($data["StudentID"],0,100);
		$showRawValues["Activity"] = substr($data["Activity"],0,100);
		$showRawValues["Level"] = substr($data["Level"],0,100);
		$showRawValues["Date_Committed"] = substr($data["Date_Committed"],0,100);
		$showRawValues["Action_Taken"] = substr($data["Action_Taken"],0,100);
		$showRawValues["Comment"] = substr($data["Comment"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_discipline";
		$options['firstTime'] = 1;
