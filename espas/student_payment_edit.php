...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_payment_inline_edit.htm" : "student_payment_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["pay_id"] = urldecode(postvalue("editid1"));
$savedKeys["pay_id"] = urldecode(postvalue("editid1"));
...
//	processing pay_date - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_pay_date = $pageObject->getControl("pay_date", $id);
		$control_pay_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pay_date - end
//	processing terms - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_terms = $pageObject->getControl("terms", $id);
		$control_terms->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing terms - end
//	processing ref_no - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_ref_no = $pageObject->getControl("ref_no", $id);
		$control_ref_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ref_no - end
	...
	//	processing amount - end
			//	processing pay_date - begin
				...
				if($condition)
				{
					$control_pay_date->afterSuccessfulSave();
				}
	//	processing pay_date - end
			//	processing terms - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_terms->afterSuccessfulSave();
				}
	//	processing terms - end
			//	processing ref_no - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_ref_no->afterSuccessfulSave();
				}
	//	processing ref_no - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["pay_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: student_payment_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_payment_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["pay_date"] = $evalues["pay_date"];
	$data["terms"] = $evalues["terms"];
	$data["ref_no"] = $evalues["ref_no"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("pay_id", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["pay_id"]));


//	pay_id - 
	$value = $pageObject->showDBValue("pay_id", $data, $keylink);
	$showValues["pay_id"] = $value;
	$showFields[] = "pay_id";
		$showRawValues["pay_id"] = substr($data["pay_id"],0,100);
...
//	pay_date - Short Date
	$value = $pageObject->showDBValue("pay_date", $data, $keylink);
	$showValues["pay_date"] = $value;
	$showFields[] = "pay_date";
		$showRawValues["pay_date"] = substr($data["pay_date"],0,100);

//	terms - 
	$value = $pageObject->showDBValue("terms", $data, $keylink);
	$showValues["terms"] = $value;
	$showFields[] = "terms";
		$showRawValues["terms"] = substr($data["terms"],0,100);

//	ref_no - 
	$value = $pageObject->showDBValue("ref_no", $data, $keylink);
	$showValues["ref_no"] = $value;
	$showFields[] = "ref_no";
		$showRawValues["ref_no"] = substr($data["ref_no"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_payment";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_payment_view.php?".$viewlink."'\"");
