...
add_nocache_headers();
include("include/enroll_apply_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["enroll_apply_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["enroll_apply_add_Maklumat_Kesihatan1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["enroll_apply_add_Maklumat_Waris1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "enroll_apply_inline_add.htm";
else
	$templatefile = "enroll_apply_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: enroll_apply_add.php');
...
//	processing at_photo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_at_photo = $pageObject->getControl("at_photo", $id);
		$control_at_photo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'enroll_apply_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'enroll_apply_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: enroll_apply_".$pageObject->getPageType().".php");
...
else
{
	...
	$defvalues["Status"] = "Belum Diproses";
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("at_photo_label","<label for=\"".GetInputElementId("at_photo", $id, PAGE_ADD)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY && $inlineadd!=ADD_POPUP)
	{
		...
		{
			$xt->assign("backToMenubutton_attrs","id=\"backToMenuButton".$id."\" onclick=\"window.location.href='menu.php';return false;\"");
			$xt->assign("backToMenu_button",true);
		}
	}
	else
	{		
...
//	show readonly fields
	$pageObject->readOnlyFields["Date_Apply"] = htmlspecialchars($pageObject->showDBValue("Date_Apply", $defvalues));
	$pageObject->readOnlyFields["Status"] = htmlspecialchars($pageObject->showDBValue("Status", $defvalues));
$linkdata="";
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "enroll_apply";
		$options['firstTime'] = 1;
