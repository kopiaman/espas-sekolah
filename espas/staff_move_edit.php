...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_move_inline_edit.htm" : "staff_move_edit.htm";
...
//	processing end - end
//	processing staff - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_staff = $pageObject->getControl("staff", $id);
		$control_staff->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing staff - end
	...
	//	processing end - end
			//	processing staff - begin
				...
				if($condition)
				{
					$control_staff->afterSuccessfulSave();
				}
	//	processing staff - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: staff_move_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_move_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	staff - 
	$value = $pageObject->showDBValue("staff", $data, $keylink);
	$showValues["staff"] = $value;
	$showFields[] = "staff";
		$showRawValues["staff"] = substr($data["staff"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_move";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_move_view.php?".$viewlink."'\"");
