...
$layout->blocks["top"][] = "details";$page_layouts["w_news_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["nid"] = urldecode(postvalue("editid1"));
$savedKeys["nid"] = urldecode(postvalue("editid1"));
...
//	processing content - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_content = $pageObject->getControl("content", $id);
		$control_content->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing content - end
//	processing ndate - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ndate = $pageObject->getControl("ndate", $id);
		$control_ndate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ndate - end
//	processing category - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_category = $pageObject->getControl("category", $id);
		$control_category->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing category - end
//	processing nstatus - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_nstatus = $pageObject->getControl("nstatus", $id);
		$control_nstatus->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nstatus - end
//	processing pic - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_pic = $pageObject->getControl("pic", $id);
		$control_pic->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pic - end
//	processing attach - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_attach = $pageObject->getControl("attach", $id);
		$control_attach->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["nid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_news_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_news_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["content"] = $evalues["content"];
	$data["ndate"] = $evalues["ndate"];
	$data["category"] = $evalues["category"];
	$data["nstatus"] = $evalues["nstatus"];
	$data["pic"] = $evalues["pic"];
	$data["attach"] = $evalues["attach"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("content_label","<label for=\"".GetInputElementId("content", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ndate_label","<label for=\"".GetInputElementId("ndate", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("category_label","<label for=\"".GetInputElementId("category", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nstatus_label","<label for=\"".GetInputElementId("nstatus", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pic_label","<label for=\"".GetInputElementId("pic", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("attach_label","<label for=\"".GetInputElementId("attach", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("nid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_news_view.php?".$viewlink."'\"");
