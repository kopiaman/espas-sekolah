...
add_nocache_headers();
include("include/job_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["job_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "job_inline_add.htm";
else
	$templatefile = "job_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing job_category - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_job_category = $pageObject->getControl("job_category", $id);
		$control_job_category->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing job_category - end
//	processing job_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_job_name = $pageObject->getControl("job_name", $id);
		$control_job_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing job_name - end
//	processing job_grade - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_job_grade = $pageObject->getControl("job_grade", $id);
		$control_job_grade->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing job_grade - end
...
//	processing job_category - start
			...
			if($inlineAddOption)
			{
				$control_job_category->afterSuccessfulSave();
			}
//	processing job_category - end
//	processing job_name - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_job_name->afterSuccessfulSave();
			}
//	processing job_name - end
//	processing job_grade - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_job_grade->afterSuccessfulSave();
			}
//	processing job_grade - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'job_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'job_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: job_".$pageObject->getPageType().".php");
...
if(postvalue("table")=="staff_info" && postvalue("field")=="Job_Title") 
	$defvalues["job_category"] = postvalue("category");
if(postvalue("table")=="staff_info" && postvalue("field")=="Grade") 
	$defvalues["job_category"] = postvalue("category");
if($readavalues)
{
	$defvalues["job_category"]=@$avalues["job_category"];
	$defvalues["job_name"]=@$avalues["job_name"];
	$defvalues["job_grade"]=@$avalues["job_grade"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("job_category"))
		$xt->assign("job_category_fieldblock",true);
	else
		$xt->assign("job_category_tabfieldblock",true);
	$xt->assign("job_category_label",true);
	if(isEnableSection508())
		$xt->assign_section("job_category_label","<label for=\"".GetInputElementId("job_category", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("job_name"))
		$xt->assign("job_name_fieldblock",true);
	else
		$xt->assign("job_name_tabfieldblock",true);
	$xt->assign("job_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("job_name_label","<label for=\"".GetInputElementId("job_name", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("job_grade"))
		$xt->assign("job_grade_fieldblock",true);
	else
		$xt->assign("job_grade_tabfieldblock",true);
	$xt->assign("job_grade_label",true);
	if(isEnableSection508())
		$xt->assign_section("job_grade_label","<label for=\"".GetInputElementId("job_grade", $id, PAGE_ADD)."\">","</label>");
...
//	job_category
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("job_category", $data, $keylink);
		$showValues["job_category"] = $value;
		$showFields[] = "job_category";
	}	
//	job_name
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("job_name", $data, $keylink);
		$showValues["job_name"] = $value;
		$showFields[] = "job_name";
	}	
//	job_grade
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("job_grade", $data, $keylink);
		$showValues["job_grade"] = $value;
		$showFields[] = "job_grade";
	}	
		$showRawValues["ID"] = substr($data["ID"],0,100);
		$showRawValues["job_category"] = substr($data["job_category"],0,100);
		$showRawValues["job_name"] = substr($data["job_name"],0,100);
		$showRawValues["job_grade"] = substr($data["job_grade"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="staff_info" && postvalue("field")=="Job_Title") 
	{
		$lookupFieldName = "Job_Title";
		$lookupTable = "job";
		$linkFieldName = "job_name";
		$dispfield = "job_name";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("staff_info", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "staff_info", "Job_Title");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("job_name", $strTableName, true);
	...
	if(postvalue("table")=="staff_info" && postvalue("field")=="Grade") 
	{
		$lookupFieldName = "Grade";
		$lookupTable = "job";
		$linkFieldName = "job_grade";
		$dispfield = "job_grade";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("staff_info", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "staff_info", "Grade");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("job_grade", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",job_grade";
		}
	}
