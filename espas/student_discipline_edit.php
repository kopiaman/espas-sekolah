...
$layout->blocks["top"][] = "details";$page_layouts["student_discipline_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_discipline_inline_edit.htm" : "student_discipline_edit.htm";
...
//	processing Activity - begin
	$condition = 1;

	if($condition)
	{
		$control_Activity = $pageObject->getControl("Activity", $id);
		$control_Activity->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Date_Committed - begin
	$condition = 1;

	if($condition)
	{
		$control_Date_Committed = $pageObject->getControl("Date_Committed", $id);
		$control_Date_Committed->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Date_Committed - end
//	processing Action_Taken - begin
	$condition = 1;

	if($condition)
	{
		$control_Action_Taken = $pageObject->getControl("Action_Taken", $id);
		$control_Action_Taken->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Action_Taken - end
//	processing Comment - begin
	$condition = 1;

	if($condition)
	{
		$control_Comment = $pageObject->getControl("Comment", $id);
		$control_Comment->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_discipline_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_discipline_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Activity"] = $evalues["Activity"];
	$data["Level"] = $evalues["Level"];
	$data["Date_Committed"] = $evalues["Date_Committed"];
	$data["Action_Taken"] = $evalues["Action_Taken"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Activity_label","<label for=\"".GetInputElementId("Activity", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Date_Committed_label","<label for=\"".GetInputElementId("Date_Committed", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Action_Taken_label","<label for=\"".GetInputElementId("Action_Taken", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Comment_label","<label for=\"".GetInputElementId("Comment", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_discipline_view.php?".$viewlink."'\"");
