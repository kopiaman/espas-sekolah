...
$layout = new TLayout("add21","Purific1Green1","MobileGreen1");
...
$layout->blocks["top"][] = "details";$page_layouts["student_info_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["student_info_add_Personal1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_info_add_Campus_Info1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_info_add_Next_Of_Kin1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_info_inline_add.htm";
else
	$templatefile = "student_info_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_info_add.php');
...
//	processing MatricNo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_MatricNo = $pageObject->getControl("MatricNo", $id);
		$control_MatricNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing sesi - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_sesi = $pageObject->getControl("sesi", $id);
		$control_sesi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sesi - end
...
//	processing Dateout - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Dateout = $pageObject->getControl("Dateout", $id);
		$control_Dateout->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing f1_job - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_f1_job = $pageObject->getControl("f1_job", $id);
		$control_f1_job->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing f2_job - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_f2_job = $pageObject->getControl("f2_job", $id);
		$control_f2_job->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_info_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_info_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_info_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["MatricNo"]=@$avalues["MatricNo"];
	...
	$defvalues["sesi"]=@$avalues["sesi"];
	$defvalues["tahun"]=@$avalues["tahun"];
	$defvalues["Dateout"]=@$avalues["Dateout"];
	$defvalues["f2_job"]=@$avalues["f2_job"];
	$defvalues["f1_job"]=@$avalues["f1_job"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("MatricNo_label","<label for=\"".GetInputElementId("MatricNo", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sesi_label","<label for=\"".GetInputElementId("sesi", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Dateout_label","<label for=\"".GetInputElementId("Dateout", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("f2_job_label","<label for=\"".GetInputElementId("f2_job", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("f1_job_label","<label for=\"".GetInputElementId("f1_job", $id, PAGE_ADD)."\">","</label>");
