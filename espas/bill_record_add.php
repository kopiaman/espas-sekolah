...
if($inlineadd == ADD_INLINE)
	$templatefile = "bill_record_inline_add.htm";
else
	$templatefile = "bill_record_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bill_record_add.php');
...
//	processing bill_date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_bill_date = $pageObject->getControl("bill_date", $id);
		$control_bill_date->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing bill_date - end
//	processing bill_no - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_bill_no = $pageObject->getControl("bill_no", $id);
		$control_bill_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing bill_no - end
...
//	processing bill_date - start
			...
			if($inlineAddOption)
			{
				$control_bill_date->afterSuccessfulSave();
			}
//	processing bill_date - end
//	processing bill_no - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_bill_no->afterSuccessfulSave();
			}
//	processing bill_no - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bill_record_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bill_record_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bill_record_".$pageObject->getPageType().".php");
...
//	bill_date
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("bill_date", $data, $keylink);
		$showValues["bill_date"] = $value;
		$showFields[] = "bill_date";
	}	
//	bill_no
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("bill_no", $data, $keylink);
		$showValues["bill_no"] = $value;
		$showFields[] = "bill_no";
	}	
//	darjah
	$display = false;
...
//	sstatus
	...
	{	
		...
		$showRawValues["bill_date"] = substr($data["bill_date"],0,100);
		$showRawValues["bill_no"] = substr($data["bill_no"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bill_record";
		$options['firstTime'] = 1;
