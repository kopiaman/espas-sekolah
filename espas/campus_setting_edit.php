...
$layout->blocks["top"][] = "details";$page_layouts["campus_setting_edit"] = $layout;
...
$templatefile = "campus_setting_edit.htm";
...
//	processing logo - begin
	...
	if($condition)
	{
		$control_logo = $pageObject->getControl("logo", $id);
		$control_logo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing gb_name - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_gb_name = $pageObject->getControl("gb_name", $id);
		$control_gb_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing gb_name - end
//	processing gb_tel - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_gb_tel = $pageObject->getControl("gb_tel", $id);
		$control_gb_tel->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing gb_tel - end
//	processing sek_mix - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sek_mix = $pageObject->getControl("sek_mix", $id);
		$control_sek_mix->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sek_mix - end
//	processing sek_kod - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sek_kod = $pageObject->getControl("sek_kod", $id);
		$control_sek_kod->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sek_kod - end
//	processing sek_building - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sek_building = $pageObject->getControl("sek_building", $id);
		$control_sek_building->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sek_building - end
//	processing sek_owner - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sek_owner = $pageObject->getControl("sek_owner", $id);
		$control_sek_owner->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sek_owner - end
//	processing sek_jenis - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sek_jenis = $pageObject->getControl("sek_jenis", $id);
		$control_sek_jenis->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sek_jenis - end
	...
	//	do event
		...
		{		
			...
			//	processing logo - begin
				...
				if($condition)
				{
					$control_logo->afterSuccessfulSave();
				}
	//	processing logo - end
			//	processing acronyim - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing daerah - end
			//	processing gb_name - begin
				...
				if($condition)
				{
					$control_gb_name->afterSuccessfulSave();
				}
	//	processing gb_name - end
			//	processing gb_tel - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_gb_tel->afterSuccessfulSave();
				}
	//	processing gb_tel - end
			//	processing sek_mix - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_sek_mix->afterSuccessfulSave();
				}
	//	processing sek_mix - end
			//	processing sek_kod - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_sek_kod->afterSuccessfulSave();
				}
	//	processing sek_kod - end
			//	processing sek_building - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_sek_building->afterSuccessfulSave();
				}
	//	processing sek_building - end
			//	processing sek_owner - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_sek_owner->afterSuccessfulSave();
				}
	//	processing sek_owner - end
			//	processing sek_jenis - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_sek_jenis->afterSuccessfulSave();
				}
	//	processing sek_jenis - end
...
if($readevalues)
{
	...
	$data["gb_name"] = $evalues["gb_name"];
	$data["gb_tel"] = $evalues["gb_tel"];
	$data["sek_mix"] = $evalues["sek_mix"];
	$data["sek_kod"] = $evalues["sek_kod"];
	$data["sek_building"] = $evalues["sek_building"];
	$data["sek_owner"] = $evalues["sek_owner"];
	$data["sek_jenis"] = $evalues["sek_jenis"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("logo_label","<label for=\"".GetInputElementId("logo", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("acronyim"))
		$xt->assign("acronyim_fieldblock",true);
	else
		$xt->assign("acronyim_tabfieldblock",true);
	$xt->assign("acronyim_label",true);
	if(isEnableSection508())
		$xt->assign_section("acronyim_label","<label for=\"".GetInputElementId("acronyim", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("campus_name"))
		$xt->assign("campus_name_fieldblock",true);
	else
		$xt->assign("campus_name_tabfieldblock",true);
	$xt->assign("campus_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("campus_name_label","<label for=\"".GetInputElementId("campus_name", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("faxNo_label","<label for=\"".GetInputElementId("faxNo", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("telephone"))
		$xt->assign("telephone_fieldblock",true);
	else
		$xt->assign("telephone_tabfieldblock",true);
	$xt->assign("telephone_label",true);
	if(isEnableSection508())
		$xt->assign_section("telephone_label","<label for=\"".GetInputElementId("telephone", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("gb_name"))
		$xt->assign("gb_name_fieldblock",true);
	else
		$xt->assign("gb_name_tabfieldblock",true);
	$xt->assign("gb_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("gb_name_label","<label for=\"".GetInputElementId("gb_name", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("gb_tel"))
		$xt->assign("gb_tel_fieldblock",true);
	else
		$xt->assign("gb_tel_tabfieldblock",true);
	$xt->assign("gb_tel_label",true);
	if(isEnableSection508())
		$xt->assign_section("gb_tel_label","<label for=\"".GetInputElementId("gb_tel", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sek_mix"))
		$xt->assign("sek_mix_fieldblock",true);
	else
		$xt->assign("sek_mix_tabfieldblock",true);
	$xt->assign("sek_mix_label",true);
	if(isEnableSection508())
		$xt->assign_section("sek_mix_label","<label for=\"".GetInputElementId("sek_mix", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sek_kod"))
		$xt->assign("sek_kod_fieldblock",true);
	else
		$xt->assign("sek_kod_tabfieldblock",true);
	$xt->assign("sek_kod_label",true);
	if(isEnableSection508())
		$xt->assign_section("sek_kod_label","<label for=\"".GetInputElementId("sek_kod", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sek_building"))
		$xt->assign("sek_building_fieldblock",true);
	else
		$xt->assign("sek_building_tabfieldblock",true);
	$xt->assign("sek_building_label",true);
	if(isEnableSection508())
		$xt->assign_section("sek_building_label","<label for=\"".GetInputElementId("sek_building", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sek_owner"))
		$xt->assign("sek_owner_fieldblock",true);
	else
		$xt->assign("sek_owner_tabfieldblock",true);
	$xt->assign("sek_owner_label",true);
	if(isEnableSection508())
		$xt->assign_section("sek_owner_label","<label for=\"".GetInputElementId("sek_owner", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sek_jenis"))
		$xt->assign("sek_jenis_fieldblock",true);
	else
		$xt->assign("sek_jenis_tabfieldblock",true);
	$xt->assign("sek_jenis_label",true);
	if(isEnableSection508())
		$xt->assign_section("sek_jenis_label","<label for=\"".GetInputElementId("sek_jenis", $id, PAGE_EDIT)."\">","</label>");
