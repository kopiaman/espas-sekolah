...
$layout = new TLayout("detailspreview","Purific1Green1","MobileGreen1");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["student_payment_detailspreview"] = $layout;
...
if($mastertable == "student_billing")
{
	$where = "";
...
if($rowcount) {
	...
	//	pay_date - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("pay_date", $data, $keylink);
			$row["pay_date_value"] = $value;
	//	terms - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("terms", $data, $keylink);
			$row["terms_value"] = $value;
	//	ref_no - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ref_no", $data, $keylink);
			$row["ref_no_value"] = $value;
	//	option - Custom
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("option", $data, $keylink);
...
$xt->load_template("student_payment_detailspreview.htm");
