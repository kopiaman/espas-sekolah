...
coalesce(sum(staff_info.Gender='L' && staff_info.Grade='KAFA'), 0) AS kafal,
coalesce(sum(staff_info.Gender='P' && staff_info.Grade='KAFA'), 0) AS kafap,
coalesce(sum(staff_info.Grade='KAFA'), 0) AS sum_kafa,

coalesce(sum(staff_info.Gender='L' && staff_info.Grade='RAKYAT'), 0) AS rakyatl,
coalesce(sum(staff_info.Gender='P' && staff_info.Grade='RAKYAT'), 0) AS rakyatp,
coalesce(sum(staff_info.Grade='RAKYAT'), 0) AS sum_rakyat,
...
COUNT(staff_info.Gender) AS guru_all
FROM staff_info
...
FROM staff_info
WHERE (staff_info.Job_Type ='Pekerja')
";
