...
if($inlineadd == ADD_INLINE)
	$templatefile = "attend_creator_staff_inline_add.htm";
else
	$templatefile = "attend_creator_staff_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: attend_creator_staff_add.php');
...
//	processing teach_day_staff - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_teach_day_staff = $pageObject->getControl("teach_day_staff", $id);
		$control_teach_day_staff->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'attend_creator_staff_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'attend_creator_staff_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: attend_creator_staff_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["aid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["aid"]=postvalue("editid1");
	}
