...
if($inlineadd == ADD_INLINE)
	$templatefile = "create_pbsra_inline_add.htm";
else
	$templatefile = "create_pbsra_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: create_pbsra_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing pyear - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_pyear = $pageObject->getControl("pyear", $id);
		$control_pyear->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pyear - end
//	processing pmonth - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_pmonth = $pageObject->getControl("pmonth", $id);
		$control_pmonth->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pmonth - end
...
//	processing pyear - start
			...
			if($inlineAddOption)
			{
				$control_pyear->afterSuccessfulSave();
			}
//	processing pyear - end
//	processing pmonth - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_pmonth->afterSuccessfulSave();
			}
//	processing pmonth - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'create_pbsra_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'create_pbsra_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: create_pbsra_".$pageObject->getPageType().".php");
...
//	pyear
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("pyear", $data, $keylink);
		$showValues["pyear"] = $value;
		$showFields[] = "pyear";
	}	
//	pmonth
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("pmonth", $data, $keylink);
		$showValues["pmonth"] = $value;
		$showFields[] = "pmonth";
	}	
//	no_student
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("no_student", $data, $keylink);
		$showValues["no_student"] = $value;
		$showFields[] = "no_student";
	}	
//	student_in
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("student_in", $data, $keylink);
		$showValues["student_in"] = $value;
		$showFields[] = "student_in";
	}	
//	student_out
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("student_out", $data, $keylink);
		$showValues["student_out"] = $value;
		$showFields[] = "student_out";
	}	
//	no_male
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("no_male", $data, $keylink);
		$showValues["no_male"] = $value;
		$showFields[] = "no_male";
	}	
//	no_female
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("no_female", $data, $keylink);
		$showValues["no_female"] = $value;
		$showFields[] = "no_female";
	}	
//	Option
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
...
//	male_out
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("male_out", $data, $keylink);
		$showValues["male_out"] = $value;
		$showFields[] = "male_out";
	}	
//	female_out
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("female_out", $data, $keylink);
		$showValues["female_out"] = $value;
		$showFields[] = "female_out";
	}	
//	male_in
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("male_in", $data, $keylink);
		$showValues["male_in"] = $value;
		$showFields[] = "male_in";
	}	
//	female_in
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("female_in", $data, $keylink);
		$showValues["female_in"] = $value;
		$showFields[] = "female_in";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["pyear"] = substr($data["pyear"],0,100);
		$showRawValues["pmonth"] = substr($data["pmonth"],0,100);
		$showRawValues["no_student"] = substr($data["no_student"],0,100);
		$showRawValues["student_in"] = substr($data["student_in"],0,100);
		$showRawValues["student_out"] = substr($data["student_out"],0,100);
		$showRawValues["no_male"] = substr($data["no_male"],0,100);
		$showRawValues["no_female"] = substr($data["no_female"],0,100);
		$showRawValues["att_male"] = substr($data["att_male"],0,100);
		$showRawValues["att_female"] = substr($data["att_female"],0,100);
		$showRawValues["Option"] = substr($data["Option"],0,100);
		$showRawValues["att_all"] = substr($data["att_all"],0,100);
		$showRawValues["male_out"] = substr($data["male_out"],0,100);
		$showRawValues["female_out"] = substr($data["female_out"],0,100);
		$showRawValues["male_in"] = substr($data["male_in"],0,100);
		$showRawValues["female_in"] = substr($data["female_in"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "create_pbsra";
		$options['firstTime'] = 1;
