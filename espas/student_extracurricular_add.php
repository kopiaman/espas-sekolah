...
add_nocache_headers();
include("include/student_extracurricular_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["student_extracurricular_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_extracurricular_inline_add.htm";
else
	$templatefile = "student_extracurricular_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_extracurricular_add.php');
...
//	processing Achievement - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Achievement = $pageObject->getControl("Achievement", $id);
		$control_Achievement->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Achievement - end
...
//	processing Achievement - start
			...
			if($inlineAddOption)
			{
				$control_Achievement->afterSuccessfulSave();
			}
//	processing Achievement - end
//	processing Start_Date - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_extracurricular_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_extracurricular_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_extracurricular_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["Achievement"]=@$avalues["Achievement"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Achievement"))
		$xt->assign("Achievement_fieldblock",true);
	else
		$xt->assign("Achievement_tabfieldblock",true);
	$xt->assign("Achievement_label",true);
	if(isEnableSection508())
		$xt->assign_section("Achievement_label","<label for=\"".GetInputElementId("Achievement", $id, PAGE_ADD)."\">","</label>");
...
//	Achievement
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Achievement", $data, $keylink);
		$showValues["Achievement"] = $value;
		$showFields[] = "Achievement";
	}	
//	Start_Date
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	Level
	...
	{	
		...
		$showRawValues["Achievement"] = substr($data["Achievement"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_extracurricular";
		$options['firstTime'] = 1;
	...
	$strTableName = "student_extracurricular";
}
/////////////////////////////////////////////////////////////
