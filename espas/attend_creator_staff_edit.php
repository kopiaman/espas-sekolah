...
include("include/attend_creator_staff_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "attend_creator_staff_inline_edit.htm" : "attend_creator_staff_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["aid"] = urldecode(postvalue("editid1"));
$savedKeys["aid"] = urldecode(postvalue("editid1"));
...
//	processing amonth - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_amonth = $pageObject->getControl("amonth", $id);
		$control_amonth->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing amonth - end
//	processing ayear - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_ayear = $pageObject->getControl("ayear", $id);
		$control_ayear->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ayear - end
//	processing teach_day_staff - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_teach_day_staff = $pageObject->getControl("teach_day_staff", $id);
		$control_teach_day_staff->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing teach_day_staff - end
	...
	//	processing ayear - end
			//	processing teach_day_staff - begin
				...
				if($condition)
				{
					$control_teach_day_staff->afterSuccessfulSave();
				}
	//	processing teach_day_staff - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["aid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: attend_creator_staff_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: attend_creator_staff_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["amonth"] = $evalues["amonth"];
	$data["ayear"] = $evalues["ayear"];
	$data["teach_day_staff"] = $evalues["teach_day_staff"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("aid", $data)));
...
//	aid - 
	$value = $pageObject->showDBValue("aid", $data, $keylink);
	$showValues["aid"] = $value;
	$showFields[] = "aid";
		$showRawValues["aid"] = substr($data["aid"],0,100);
...
//	teach_day_staff - 
	$value = $pageObject->showDBValue("teach_day_staff", $data, $keylink);
	$showValues["teach_day_staff"] = $value;
	$showFields[] = "teach_day_staff";
		$showRawValues["teach_day_staff"] = substr($data["teach_day_staff"],0,100);

//	teach_day - 
	$value = $pageObject->showDBValue("teach_day", $data, $keylink);
	$showValues["teach_day"] = $value;
	$showFields[] = "teach_day";
		$showRawValues["teach_day"] = substr($data["teach_day"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "attend_creator_staff";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='attend_creator_staff_view.php?".$viewlink."'\"");
