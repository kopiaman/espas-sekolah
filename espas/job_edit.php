...
$layout->blocks["top"][] = "details";$page_layouts["job_edit"] = $layout;
...
//	processing job_category - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_job_category = $pageObject->getControl("job_category", $id);
		$control_job_category->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing job_category - end
//	processing job_name - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_job_name = $pageObject->getControl("job_name", $id);
		$control_job_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing job_name - end
//	processing job_grade - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_job_grade = $pageObject->getControl("job_grade", $id);
		$control_job_grade->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: job_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: job_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["job_category"] = $evalues["job_category"];
	$data["job_name"] = $evalues["job_name"];
	$data["job_grade"] = $evalues["job_grade"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("job_category_label","<label for=\"".GetInputElementId("job_category", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("job_name_label","<label for=\"".GetInputElementId("job_name", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("job_grade_label","<label for=\"".GetInputElementId("job_grade", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='job_view.php?".$viewlink."'\"");
