...
$layout->blocks["top"][] = "details";$page_layouts["w_news_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_news_inline_add.htm";
else
	$templatefile = "w_news_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: w_news_add.php');
...
//	processing ndate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ndate = $pageObject->getControl("ndate", $id);
		$control_ndate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ndate - end
...
//	processing nstatus - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_nstatus = $pageObject->getControl("nstatus", $id);
		$control_nstatus->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nstatus - end
//	processing pic - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_pic = $pageObject->getControl("pic", $id);
		$control_pic->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pic - end
//	processing attach - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_attach = $pageObject->getControl("attach", $id);
		$control_attach->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing attach - end
//	processing content - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_content = $pageObject->getControl("content", $id);
		$control_content->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing ndate - start
			...
			if($inlineAddOption)
			{
				$control_ndate->afterSuccessfulSave();
			}
//	processing ndate - end
//	processing category - start
			$inlineAddOption = true;
...
//	processing nstatus - start
			...
			if($inlineAddOption)
			{
				$control_nstatus->afterSuccessfulSave();
			}
//	processing nstatus - end
//	processing pic - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_pic->afterSuccessfulSave();
			}
//	processing pic - end
//	processing attach - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_attach->afterSuccessfulSave();
			}
//	processing attach - end
//	processing content - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_content->afterSuccessfulSave();
			}
//	processing content - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_news_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_news_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_news_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["nid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["nid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["content"]=@$avalues["content"];
	$defvalues["ndate"]=@$avalues["ndate"];
	$defvalues["category"]=@$avalues["category"];
	$defvalues["nstatus"]=@$avalues["nstatus"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("content"))
		$xt->assign("content_fieldblock",true);
	else
		$xt->assign("content_tabfieldblock",true);
	$xt->assign("content_label",true);
	if(isEnableSection508())
		$xt->assign_section("content_label","<label for=\"".GetInputElementId("content", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ndate"))
		$xt->assign("ndate_fieldblock",true);
	else
		$xt->assign("ndate_tabfieldblock",true);
	$xt->assign("ndate_label",true);
	if(isEnableSection508())
		$xt->assign_section("ndate_label","<label for=\"".GetInputElementId("ndate", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("nstatus"))
		$xt->assign("nstatus_fieldblock",true);
	else
		$xt->assign("nstatus_tabfieldblock",true);
	$xt->assign("nstatus_label",true);
	if(isEnableSection508())
		$xt->assign_section("nstatus_label","<label for=\"".GetInputElementId("nstatus", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("pic"))
		$xt->assign("pic_fieldblock",true);
	else
		$xt->assign("pic_tabfieldblock",true);
	$xt->assign("pic_label",true);
	if(isEnableSection508())
		$xt->assign_section("pic_label","<label for=\"".GetInputElementId("pic", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("attach"))
		$xt->assign("attach_fieldblock",true);
	else
		$xt->assign("attach_tabfieldblock",true);
	$xt->assign("attach_label",true);
	if(isEnableSection508())
		$xt->assign_section("attach_label","<label for=\"".GetInputElementId("attach", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["nid"]));
...
//	content
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("content", $data, $keylink);
		$showValues["content"] = $value;
		$showFields[] = "content";
	}	
//	ndate
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ndate", $data, $keylink);
		$showValues["ndate"] = $value;
		$showFields[] = "ndate";
	}	
//	category
	$display = false;
...
//	nstatus
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("nstatus", $data, $keylink);
		$showValues["nstatus"] = $value;
		$showFields[] = "nstatus";
	}	
//	pic
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("pic", $data, $keylink);
		$showValues["pic"] = $value;
		$showFields[] = "pic";
	}	
//	attach
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("attach", $data, $keylink);
		$showValues["attach"] = $value;
		$showFields[] = "attach";
	}	
		$showRawValues["nid"] = substr($data["nid"],0,100);
		$showRawValues["title"] = substr($data["title"],0,100);
		$showRawValues["content"] = substr($data["content"],0,100);
		$showRawValues["ndate"] = substr($data["ndate"],0,100);
		$showRawValues["category"] = substr($data["category"],0,100);
		$showRawValues["nstatus"] = substr($data["nstatus"],0,100);
		$showRawValues["pic"] = substr($data["pic"],0,100);
		$showRawValues["attach"] = substr($data["attach"],0,100);
