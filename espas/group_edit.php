...
include("include/group_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "group_inline_edit.htm" : "group_edit.htm";
...
//	processing group - begin
	...
	if($condition)
	{
		$control_group = $pageObject->getControl("group", $id);
		$control_group->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing group - begin
				...
				if($condition)
				{
					$control_group->afterSuccessfulSave();
				}
	//	processing group - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: group_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: group_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	group - 
	$value = $pageObject->showDBValue("group", $data, $keylink);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='group_view.php?".$viewlink."'\"");
