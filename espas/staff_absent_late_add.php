...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_absent_late_inline_add.htm";
else
	$templatefile = "staff_absent_late_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_absent_late_add.php');
...
//	processing abdate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_abdate = $pageObject->getControl("abdate", $id);
		$control_abdate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing reason - start
	...
	if($inlineAddOption)
	{
		...
		$control_reason->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_absent_late_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_absent_late_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_absent_late_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["abid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["abid"]=postvalue("editid1");
	}
