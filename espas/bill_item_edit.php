...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bill_item_inline_edit.htm" : "bill_item_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["itemid"] = urldecode(postvalue("editid1"));
$savedKeys["itemid"] = urldecode(postvalue("editid1"));
...
//	processing bname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_bname = $pageObject->getControl("bname", $id);
		$control_bname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing bname - end
	...
	//	do event
		...
		{		
			...
			//	processing bname - begin
				...
				if($condition)
				{
					$control_bname->afterSuccessfulSave();
				}
	//	processing bname - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bill_item_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bill_item_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("itemid", $data)));
...
//	bname - 
	$value = $pageObject->showDBValue("bname", $data, $keylink);
	$showValues["bname"] = $value;
	$showFields[] = "bname";
		$showRawValues["bname"] = substr($data["bname"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bill_item_view.php?".$viewlink."'\"");
