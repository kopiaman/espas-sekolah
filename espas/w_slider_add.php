...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_slider_inline_add.htm";
else
	$templatefile = "w_slider_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: w_slider_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing headline - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_headline = $pageObject->getControl("headline", $id);
		$control_headline->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing slink - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_slink = $pageObject->getControl("slink", $id);
		$control_slink->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_slider_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_slider_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_slider_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["slid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["slid"]=postvalue("editid1");
	}
