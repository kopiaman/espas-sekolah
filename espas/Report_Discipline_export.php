...
$layout->blocks["top"][] = "export";$page_layouts["Report_Discipline_export"] = $layout;
...
$xt->display("Report_Discipline_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Discipline.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Discipline.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Discipline.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["darjah"] = $pageObject->showDBValue("darjah", $row);
			$values["Activity"] = $pageObject->showDBValue("Activity", $row);
			$values["Level"] = $pageObject->showDBValue("Level", $row);
			$values["Date_Committed"] = $pageObject->showDBValue("Date_Committed", $row);
			$values["Action_Taken"] = $pageObject->showDBValue("Action_Taken", $row);
			$values["Comment"] = $pageObject->showDBValue("Comment", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Discipline.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["darjah"] = $pageObject->getViewControl("darjah")->showDBValue($row, "");
			$values["Activity"] = $pageObject->getViewControl("Activity")->showDBValue($row, "");
			$values["Level"] = $pageObject->getViewControl("Level")->showDBValue($row, "");
			$values["Date_Committed"] = $pageObject->getViewControl("Date_Committed")->showDBValue($row, "");
			$values["Action_Taken"] = $pageObject->getViewControl("Action_Taken")->showDBValue($row, "");
			$values["Comment"] = $pageObject->getViewControl("Comment")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["darjah"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Activity"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Level"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Date_Committed"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Action_Taken"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Comment"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nama Pelajar").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Aktiviti").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Peringkat").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tarikh Perlakuan").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tindakan").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Catatan").'</td>';	
	}
	else
	{
		echo "<td>"."Nama Pelajar"."</td>";
		echo "<td>"."Sesi"."</td>";
		echo "<td>"."Kelas"."</td>";
		echo "<td>"."Status"."</td>";
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["StudentID"]);
				else
					echo $values["StudentID"];//echo htmlspecialchars($values["StudentID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Activity"]);
					else
						echo $values["Activity"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["Level"]);
				else
					echo $values["Level"];//echo htmlspecialchars($values["Level"]); commented for bug #6823
					
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Date_Committed"]);
					else
						echo $values["Date_Committed"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["Action_Taken"]);
				else
					echo $values["Action_Taken"];//echo htmlspecialchars($values["Action_Taken"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Comment"]);
					else
						echo $values["Comment"];
			echo '</td>';
			echo "</tr>";
