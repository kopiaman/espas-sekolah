...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=Ausername\"";
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `user_ugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `user_ugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `user_ugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `user_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$row["displayusername"] = htmlspecialchars($data[""]); //htmlspecialchars($data["username"]);
	...
	{
		//	select members list		
		$trs = db_query("select `UserName`,`GroupID` from `user_ugmembers` order by `UserName`,`GroupID`",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select `GroupID`,`Label` from `user_uggroups` order by `Label`",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "username" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) ==""){
