...
include("include/staff_move_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["staff_move_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "staff_move_search.htm";
...
$params['shortTableName'] = 'staff_move';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("staff"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("staff")] = GetTableURL($pageObject->pSet->getLookupTable("staff"));
	
	$pageObject->fillFieldToolTips("staff");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("staff");
	...
	if(isEnableSection508())
		$xt->assign_section("staff_label","<label for=\"".GetInputElementId("staff", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("staff_label", true);
	
	$xt->assign("staff_fieldblock", true);
	$xt->assignbyref("staff_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("staff_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("staff_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_staff", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("staff");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"staff", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"staff", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
