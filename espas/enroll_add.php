...
$layout->blocks["top"][] = "details";$page_layouts["enroll_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["enroll_add_Maklumat_Kesihatan1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["enroll_add_Maklumat_Waris1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "enroll_inline_add.htm";
else
	$templatefile = "enroll_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: enroll_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'enroll_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'enroll_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: enroll_".$pageObject->getPageType().".php");
