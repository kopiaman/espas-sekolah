...
if($inlineadd == ADD_INLINE)
	$templatefile = "course_inline_add.htm";
else
	$templatefile = "course_add.htm";
...
//	processing Code - start
	...
	if($inlineAddOption)
	{
		...
		$control_Code->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Code - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'course_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'course_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: course_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["CourseID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["CourseID"]=postvalue("editid1");
	}
