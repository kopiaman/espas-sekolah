...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_course_inline_edit.htm" : "student_course_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["scid"] = urldecode(postvalue("editid1"));
$savedKeys["scid"] = urldecode(postvalue("editid1"));
...
//	processing s1 - begin
	...
	if($condition)
	{
		$control_s1 = $pageObject->getControl("s1", $id);
		$control_s1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s2 - begin
	...
	if($condition)
	{
		$control_s2 = $pageObject->getControl("s2", $id);
		$control_s2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s3 - begin
	...
	if($condition)
	{
		$control_s3 = $pageObject->getControl("s3", $id);
		$control_s3->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s4 - begin
	...
	if($condition)
	{
		$control_s4 = $pageObject->getControl("s4", $id);
		$control_s4->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s5 - begin
	...
	if($condition)
	{
		$control_s5 = $pageObject->getControl("s5", $id);
		$control_s5->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s6 - begin
	...
	if($condition)
	{
		$control_s6 = $pageObject->getControl("s6", $id);
		$control_s6->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s7 - begin
	...
	if($condition)
	{
		$control_s7 = $pageObject->getControl("s7", $id);
		$control_s7->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s8 - begin
	...
	if($condition)
	{
		$control_s8 = $pageObject->getControl("s8", $id);
		$control_s8->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing s9 - begin
	...
	if($condition)
	{
		$control_s9 = $pageObject->getControl("s9", $id);
		$control_s9->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing s1 - begin
				...
				if($condition)
				{
					$control_s1->afterSuccessfulSave();
				}
	//	processing s1 - end
			//	processing s2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s2->afterSuccessfulSave();
				}
	//	processing s2 - end
			//	processing s3 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s3->afterSuccessfulSave();
				}
	//	processing s3 - end
			//	processing s4 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s4->afterSuccessfulSave();
				}
	//	processing s4 - end
			//	processing s5 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s5->afterSuccessfulSave();
				}
	//	processing s5 - end
			//	processing s6 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s6->afterSuccessfulSave();
				}
	//	processing s6 - end
			//	processing s7 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s7->afterSuccessfulSave();
				}
	//	processing s7 - end
			//	processing s8 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s8->afterSuccessfulSave();
				}
	//	processing s8 - end
			//	processing s9 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_s9->afterSuccessfulSave();
				}
	//	processing s9 - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["scid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: student_course_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_course_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("scid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["scid"]));


//	scid - 
	$value = $pageObject->showDBValue("scid", $data, $keylink);
	$showValues["scid"] = $value;
	$showFields[] = "scid";
		$showRawValues["scid"] = substr($data["scid"],0,100);
...
//	s1 - 
	$value = $pageObject->showDBValue("s1", $data, $keylink);
	$showValues["s1"] = $value;
	$showFields[] = "s1";
		$showRawValues["s1"] = substr($data["s1"],0,100);

//	s2 - 
	$value = $pageObject->showDBValue("s2", $data, $keylink);
	$showValues["s2"] = $value;
	$showFields[] = "s2";
		$showRawValues["s2"] = substr($data["s2"],0,100);

//	s3 - 
	$value = $pageObject->showDBValue("s3", $data, $keylink);
	$showValues["s3"] = $value;
	$showFields[] = "s3";
		$showRawValues["s3"] = substr($data["s3"],0,100);

//	s4 - 
	$value = $pageObject->showDBValue("s4", $data, $keylink);
	$showValues["s4"] = $value;
	$showFields[] = "s4";
		$showRawValues["s4"] = substr($data["s4"],0,100);

//	s5 - 
	$value = $pageObject->showDBValue("s5", $data, $keylink);
	$showValues["s5"] = $value;
	$showFields[] = "s5";
		$showRawValues["s5"] = substr($data["s5"],0,100);

//	s6 - 
	$value = $pageObject->showDBValue("s6", $data, $keylink);
	$showValues["s6"] = $value;
	$showFields[] = "s6";
		$showRawValues["s6"] = substr($data["s6"],0,100);

//	s7 - 
	$value = $pageObject->showDBValue("s7", $data, $keylink);
	$showValues["s7"] = $value;
	$showFields[] = "s7";
		$showRawValues["s7"] = substr($data["s7"],0,100);

//	s8 - 
	$value = $pageObject->showDBValue("s8", $data, $keylink);
	$showValues["s8"] = $value;
	$showFields[] = "s8";
		$showRawValues["s8"] = substr($data["s8"],0,100);

//	s9 - 
	$value = $pageObject->showDBValue("s9", $data, $keylink);
	$showValues["s9"] = $value;
	$showFields[] = "s9";
		$showRawValues["s9"] = substr($data["s9"],0,100);

//	sum - Number
	$value = $pageObject->showDBValue("sum", $data, $keylink);
	$showValues["sum"] = $value;
	$showFields[] = "sum";
		$showRawValues["sum"] = substr($data["sum"],0,100);

//	etype - 
	$value = $pageObject->showDBValue("etype", $data, $keylink);
	$showValues["etype"] = $value;
	$showFields[] = "etype";
		$showRawValues["etype"] = substr($data["etype"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_course";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_course_view.php?".$viewlink."'\"");
