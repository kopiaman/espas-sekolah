...
$layout->blocks["top"][] = "search";$page_layouts["staff_absent_late_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "staff_absent_late_search.htm";
...
$params['shortTableName'] = 'staff_absent_late';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("abdate"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("abdate")] = GetTableURL($pageObject->pSet->getLookupTable("abdate"));
	
	$pageObject->fillFieldToolTips("abdate");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("abdate");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("abdate_label","<label for=\"".GetInputElementId("abdate", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("abdate_label", true);
	
	$xt->assign("abdate_fieldblock", true);
	$xt->assignbyref("abdate_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("abdate_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("abdate_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_abdate", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("abdate");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"abdate", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"abdate", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
