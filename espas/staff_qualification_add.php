...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_qualification_inline_add.htm";
else
	$templatefile = "staff_qualification_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_qualification_add.php');
...
//	processing YearGrad - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_YearGrad = $pageObject->getControl("YearGrad", $id);
		$control_YearGrad->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing YearGrad - end
//	processing EducationType - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_EducationType = $pageObject->getControl("EducationType", $id);
		$control_EducationType->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing EducationType - end
//	processing Major - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Major = $pageObject->getControl("Major", $id);
		$control_Major->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Major - end
//	processing Grade_CGPA - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Grade_CGPA = $pageObject->getControl("Grade_CGPA", $id);
		$control_Grade_CGPA->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Grade_CGPA - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="staff_info")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["StaffID"]==""){
			$avalues["StaffID"] = prepare_for_db("StaffID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_qualification_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_qualification_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_qualification_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="staff_info")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["StaffID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
