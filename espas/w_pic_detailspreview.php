...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["w_pic_detailspreview"] = $layout;
...
if($mastertable == "w_gallery_sub")
{
	$where = "";
		$where .= GetFullFieldName("wig", $strTableName, false)."=".make_db_value("wig",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	files - Document Download
		    ...
		    $value = $viewContainer->showDBValue("files", $data, $keylink);
			$row["files_value"] = $value;
	//	caption - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("caption", $data, $keylink);
			$row["caption_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("w_pic_detailspreview.htm");
