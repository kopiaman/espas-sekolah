...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_qualification_inline_edit.htm" : "staff_qualification_edit.htm";
...
//	processing YearGrad - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_YearGrad = $pageObject->getControl("YearGrad", $id);
		$control_YearGrad->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing YearGrad - end
//	processing EducationType - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_EducationType = $pageObject->getControl("EducationType", $id);
		$control_EducationType->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing EducationType - end
//	processing Major - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Major = $pageObject->getControl("Major", $id);
		$control_Major->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Major - end
//	processing Grade_CGPA - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Grade_CGPA = $pageObject->getControl("Grade_CGPA", $id);
		$control_Grade_CGPA->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Grade_CGPA - end
	...
	//	processing Institution - end
			//	processing YearGrad - begin
				...
				if($condition)
				{
					$control_YearGrad->afterSuccessfulSave();
				}
	//	processing YearGrad - end
			//	processing EducationType - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_EducationType->afterSuccessfulSave();
				}
	//	processing EducationType - end
			//	processing Major - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Major->afterSuccessfulSave();
				}
	//	processing Major - end
			//	processing Grade_CGPA - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Grade_CGPA->afterSuccessfulSave();
				}
	//	processing Grade_CGPA - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: staff_qualification_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_qualification_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["YearGrad"] = $evalues["YearGrad"];
	$data["EducationType"] = $evalues["EducationType"];
	$data["Major"] = $evalues["Major"];
	$data["Grade_CGPA"] = $evalues["Grade_CGPA"];
}
...
//	YearGrad - 
	$value = $pageObject->showDBValue("YearGrad", $data, $keylink);
	$showValues["YearGrad"] = $value;
	$showFields[] = "YearGrad";
		$showRawValues["YearGrad"] = substr($data["YearGrad"],0,100);

//	EducationType - 
	$value = $pageObject->showDBValue("EducationType", $data, $keylink);
	$showValues["EducationType"] = $value;
	$showFields[] = "EducationType";
		$showRawValues["EducationType"] = substr($data["EducationType"],0,100);

//	Major - 
	$value = $pageObject->showDBValue("Major", $data, $keylink);
	$showValues["Major"] = $value;
	$showFields[] = "Major";
		$showRawValues["Major"] = substr($data["Major"],0,100);

//	Grade_CGPA - 
	$value = $pageObject->showDBValue("Grade_CGPA", $data, $keylink);
	$showValues["Grade_CGPA"] = $value;
	$showFields[] = "Grade_CGPA";
		$showRawValues["Grade_CGPA"] = substr($data["Grade_CGPA"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_qualification";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_qualification_view.php?".$viewlink."'\"");
