...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_payment_inline_add.htm";
else
	$templatefile = "student_payment_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_payment_add.php');
...
//	processing pay_date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_pay_date = $pageObject->getControl("pay_date", $id);
		$control_pay_date->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing terms - start
	...
	if($inlineAddOption)
	{
		...
		$control_terms->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing terms - end
//	processing ref_no - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ref_no = $pageObject->getControl("ref_no", $id);
		$control_ref_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ref_no - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_billing")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_payment_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_payment_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_payment_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["pay_id"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["pay_id"]=postvalue("editid1");
	}
...
{
	$defvalues["pay_date"] = now();
	$defvalues["terms"] = "Tunai";
}
