...
$layout->blocks["top"][] = "search";$page_layouts["Report_Extracurricular_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_Extracurricular_search.htm";
...
$params['shortTableName'] = 'Report_Extracurricular';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Achievement"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Achievement")] = GetTableURL($pageObject->pSet->getLookupTable("Achievement"));
	
	$pageObject->fillFieldToolTips("Achievement");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Achievement");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Achievement";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Achievement_label","<label for=\"".GetInputElementId("Achievement", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Achievement_label", true);
	
	$xt->assign("Achievement_fieldblock", true);
	$xt->assignbyref("Achievement_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Achievement_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Achievement_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Achievement", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Achievement");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Achievement", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Achievement", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
