...
include("include/course_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "course_inline_edit.htm" : "course_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["CourseID"] = urldecode(postvalue("editid1"));
$savedKeys["CourseID"] = urldecode(postvalue("editid1"));
...
//	processing Code - begin
	...
	if($condition)
	{
		$control_Code = $pageObject->getControl("Code", $id);
		$control_Code->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Code - end
	...
	//	processing Name - end
			//	processing Code - begin
				...
				if($condition)
				{
					$control_Code->afterSuccessfulSave();
				}
	//	processing Code - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["CourseID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: course_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: course_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("CourseID", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["CourseID"]));


//	CourseID - 
	$value = $pageObject->showDBValue("CourseID", $data, $keylink);
	$showValues["CourseID"] = $value;
	$showFields[] = "CourseID";
		$showRawValues["CourseID"] = substr($data["CourseID"],0,100);
...
//	Code - 
	$value = $pageObject->showDBValue("Code", $data, $keylink);
	$showValues["Code"] = $value;
	$showFields[] = "Code";
		$showRawValues["Code"] = substr($data["Code"],0,100);

//	Credit - 
	$value = $pageObject->showDBValue("Credit", $data, $keylink);
	$showValues["Credit"] = $value;
	$showFields[] = "Credit";
		$showRawValues["Credit"] = substr($data["Credit"],0,100);

//	Unit_grade - Number
	$value = $pageObject->showDBValue("Unit_grade", $data, $keylink);
	$showValues["Unit_grade"] = $value;
	$showFields[] = "Unit_grade";
		$showRawValues["Unit_grade"] = substr($data["Unit_grade"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='course_view.php?".$viewlink."'\"");
