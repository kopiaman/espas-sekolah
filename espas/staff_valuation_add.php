...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_valuation_inline_add.htm";
else
	$templatefile = "staff_valuation_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_valuation_add.php');
...
//	processing rate - start
	...
	if($inlineAddOption)
	{
		...
		$control_rate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing rate - end
//	processing sdate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_sdate = $pageObject->getControl("sdate", $id);
		$control_sdate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing remark - start
	...
	if($inlineAddOption)
	{
		...
		$control_remark->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_valuation_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_valuation_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_valuation_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["svid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["svid"]=postvalue("editid1");
	}
