...
$layout->blocks["top"][] = "search";$page_layouts["Report_Staff_Kin_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_Staff_Kin_search.htm";
...
$params['shortTableName'] = 'Report_Staff_Kin';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("StaffNo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("StaffNo")] = GetTableURL($pageObject->pSet->getLookupTable("StaffNo"));
	
	$pageObject->fillFieldToolTips("StaffNo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("StaffNo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "StaffNo";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("StaffNo_label","<label for=\"".GetInputElementId("StaffNo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("StaffNo_label", true);
	
	$xt->assign("StaffNo_fieldblock", true);
	$xt->assignbyref("StaffNo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("StaffNo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("StaffNo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_StaffNo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("StaffNo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"StaffNo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"StaffNo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
