...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_upsra_inline_add.htm";
else
	$templatefile = "student_upsra_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_upsra_add.php');
...
//	processing icno - end
//	processing syear - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_syear = $pageObject->getControl("syear", $id);
		$control_syear->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing syear - end
...
//	processing syear - start
			...
			if($inlineAddOption)
			{
				$control_syear->afterSuccessfulSave();
			}
//	processing syear - end
//	processing sstatus - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_upsra_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_upsra_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_upsra_".$pageObject->getPageType().".php");
...
//	syear
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("syear", $data, $keylink);
		$showValues["syear"] = $value;
		$showFields[] = "syear";
	}	
//	sstatus
	$display = false;
	...
	{	
		...
		$showRawValues["scode"] = substr($data["scode"],0,100);
		$showRawValues["icno"] = substr($data["icno"],0,100);
		$showRawValues["syear"] = substr($data["syear"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_upsra";
		$options['firstTime'] = 1;
