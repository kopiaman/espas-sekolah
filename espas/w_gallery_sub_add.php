...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_gallery_sub_inline_add.htm";
else
	$templatefile = "w_gallery_sub_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: w_gallery_sub_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_gallery_sub_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_gallery_sub_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_gallery_sub_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["wig"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["wig"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="w_gallery")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["wcat"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
