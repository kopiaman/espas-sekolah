...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("MedicalCondition"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("MedicalCondition")] = GetTableURL($pageObject->pSet->getLookupTable("MedicalCondition"));
	
	$pageObject->fillFieldToolTips("MedicalCondition");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("MedicalCondition");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "MedicalCondition";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("MedicalCondition_label","<label for=\"".GetInputElementId("MedicalCondition", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("MedicalCondition_label", true);
	
	$xt->assign("MedicalCondition_fieldblock", true);
	$xt->assignbyref("MedicalCondition_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("MedicalCondition_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("MedicalCondition_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_MedicalCondition", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("MedicalCondition");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MedicalCondition", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MedicalCondition", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
