...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_billing_inline_add.htm";
else
	$templatefile = "student_billing_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_billing_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_billing_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_billing_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_billing_".$pageObject->getPageType().".php");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["student_payment"]["masterkey1"] = $data["sid"];	
...
//	paid_amount
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("paid_amount", $data, $keylink);
		$showValues["paid_amount"] = $value;
		$showFields[] = "paid_amount";
	}	
//	cetak
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("cetak", $data, $keylink);
		$showValues["cetak"] = $value;
		$showFields[] = "cetak";
	}	
//	balance
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("balance", $data, $keylink);
		$showValues["balance"] = $value;
		$showFields[] = "balance";
	}	
//	sstatus
	$display = false;
...
//	Class
	...
	{	
		...
		$showRawValues["paid_amount"] = substr($data["paid_amount"],0,100);
		$showRawValues["cetak"] = substr($data["cetak"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_billing";
		$options['firstTime'] = 1;
