...
$layout->blocks["top"][] = "search";$page_layouts["course_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "course_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Code"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Code")] = GetTableURL($pageObject->pSet->getLookupTable("Code"));
	...
	if(isEnableSection508())
		$xt->assign_section("Code_label","<label for=\"".GetInputElementId("Code", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Code_label", true);
	
	$xt->assign("Code_fieldblock", true);
	$xt->assignbyref("Code_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Code_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
