...
include("include/w_gallery_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "w_gallery_inline_edit.htm" : "w_gallery_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["wcat"] = urldecode(postvalue("editid1"));
$savedKeys["wcat"] = urldecode(postvalue("editid1"));
...
//	processing category_name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_category_name = $pageObject->getControl("category_name", $id);
		$control_category_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["wcat"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_gallery_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_gallery_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["category_name"] = $evalues["category_name"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("wcat", $data)));
...
//	detail tables
	$showDetailKeys["w_gallery_sub"]["masterkey1"] = $data["wcat"];		

	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["wcat"]));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_gallery_view.php?".$viewlink."'\"");
