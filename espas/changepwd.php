...
$layout = new TLayout("changepwd","Purific1Green1","MobileGreen1");
		...
		if($row=$cipherer->DecryptFetchedArray($rstemp))
		{
			if($opass == $row[$cPasswordField])
			{
				...
				if($retval)
				{
					...
					$layout = new TLayout("changepwd_success","Purific1Green1","MobileGreen1");
$layout->blocks["top"] = array();
$layout->containers["fields"] = array();
...
function validate(){
	if (document.forms.form1.cpass.value!=document.forms.form1.newpass.value){	
		alert('".jsreplace("Katalaluan tidak padan. Masukkan katalaluan semula").
		"');
