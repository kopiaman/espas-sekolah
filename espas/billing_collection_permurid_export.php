...
$layout->blocks["top"][] = "export";$page_layouts["billing_collection_permurid_export"] = $layout;
...
$xt->display("billing_collection_permurid_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=billing_collection_permurid.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=billing_collection_permurid.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=billing_collection_permurid.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Bill"] = $pageObject->showDBValue("Bill", $row);
			$values["Paid"] = $pageObject->showDBValue("Paid", $row);
			$values["Outstanding"] = $pageObject->showDBValue("Outstanding", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=billing_collection_permurid.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Bill"] = $row["Bill"];
			$values["Paid"] = $row["Paid"];
			$values["Outstanding"] = $row["Outstanding"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Bill"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Paid"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Outstanding"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Yuran").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Dibayar").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tertunggak").'</td>';	
	}
	else
	{
		echo "<td>"."Nama"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["tahun"] = $pageObject->getViewControl("tahun")->showDBValue($row, "");
					$values["sesi"] = $pageObject->getViewControl("sesi")->showDBValue($row, "");
					$values["Bill"] = $pageObject->getViewControl("Bill")->showDBValue($row, "");
					$values["Paid"] = $pageObject->getViewControl("Paid")->showDBValue($row, "");
					$values["Outstanding"] = $pageObject->getViewControl("Outstanding")->showDBValue($row, "");
