...
include("include/w_slider_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "w_slider_inline_edit.htm" : "w_slider_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["slid"] = urldecode(postvalue("editid1"));
$savedKeys["slid"] = urldecode(postvalue("editid1"));
...
//	processing headline - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_headline = $pageObject->getControl("headline", $id);
		$control_headline->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing headline - end
...
//	processing slink - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_slink = $pageObject->getControl("slink", $id);
		$control_slink->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing slink - end
...
//	processing sstatus - end
	...
	//	do event
		...
		{		
			...
			//	processing headline - begin
				...
				if($condition)
				{
					$control_headline->afterSuccessfulSave();
				}
	//	processing headline - end
			//	processing picture - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing picture - end
			//	processing slink - begin
				...
				if($condition)
				{
					$control_slink->afterSuccessfulSave();
				}
	//	processing slink - end
			//	processing sstatus - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["slid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_slider_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_slider_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["headline"] = $evalues["headline"];
	$data["picture"] = $evalues["picture"];
	$data["slink"] = $evalues["slink"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("slid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["slid"]));


//	slid - 
	$value = $pageObject->showDBValue("slid", $data, $keylink);
	$showValues["slid"] = $value;
	$showFields[] = "slid";
		$showRawValues["slid"] = substr($data["slid"],0,100);

//	headline - 
	$value = $pageObject->showDBValue("headline", $data, $keylink);
	$showValues["headline"] = $value;
	$showFields[] = "headline";
		$showRawValues["headline"] = substr($data["headline"],0,100);

//	picture - Document Download
...
//	slink - 
	$value = $pageObject->showDBValue("slink", $data, $keylink);
	$showValues["slink"] = $value;
	$showFields[] = "slink";
		$showRawValues["slink"] = substr($data["slink"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_slider_view.php?".$viewlink."'\"");
