...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["w_gallery_sub_detailspreview"] = $layout;
...
if($mastertable == "w_gallery")
{
	$where = "";
		$where .= GetFullFieldName("wcat", $strTableName, false)."=".make_db_value("wcat",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	sub_cat - 
		    ...
		    $value = $viewContainer->showDBValue("sub_cat", $data, $keylink);
			$row["sub_cat_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("w_gallery_sub_detailspreview.htm");
