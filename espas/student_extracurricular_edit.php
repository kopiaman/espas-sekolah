...
$layout->blocks["top"][] = "details";$page_layouts["student_extracurricular_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_extracurricular_inline_edit.htm" : "student_extracurricular_edit.htm";
...
//	processing Achievement - begin
	$condition = 1;

	if($condition)
	{
		$control_Achievement = $pageObject->getControl("Achievement", $id);
		$control_Achievement->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_extracurricular_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_extracurricular_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Achievement"] = $evalues["Achievement"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("StudentID_label","<label for=\"".GetInputElementId("StudentID", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Achievement_label","<label for=\"".GetInputElementId("Achievement", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_extracurricular_view.php?".$viewlink."'\"");
