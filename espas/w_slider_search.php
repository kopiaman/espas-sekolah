...
$layout->blocks["top"][] = "search";$page_layouts["w_slider_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "w_slider_search.htm";
...
$params['shortTableName'] = 'w_slider';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("headline"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("headline")] = GetTableURL($pageObject->pSet->getLookupTable("headline"));
	
	$pageObject->fillFieldToolTips("headline");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("headline");
	...
	{
		$firstFieldParams['fName'] = "headline";
	...
	if(isEnableSection508())
		$xt->assign_section("headline_label","<label for=\"".GetInputElementId("headline", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("headline_label", true);
	
	$xt->assign("headline_fieldblock", true);
	$xt->assignbyref("headline_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("headline_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("headline_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_headline", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("headline");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"headline", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"headline", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
