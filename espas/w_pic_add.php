...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_pic_inline_add.htm";
else
	$templatefile = "w_pic_add.htm";
...
//	processing caption - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_caption = $pageObject->getControl("caption", $id);
		$control_caption->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing caption - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="w_gallery_sub")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["wig"]==""){
			$avalues["wig"] = prepare_for_db("wig",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_pic_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_pic_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_pic_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["picid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["picid"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="w_gallery_sub")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["wig"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
